...
import { llmDiscoverModelsValueSchema, llmModelsValueSchema, llmProvidersValueSchema, llmSubscriptionUsageValueSchema } from '../api/llm.schema.ts'
import {
...
export interface IApiClient {
  ...
  llm: {
    ...
    subscriptionUsage(payload: RequestPayload<'llm.subscriptionUsage'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'llm.subscriptionUsage'>>>
  }
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'llm.subscriptionUsage': llmSubscriptionUsageValueSchema,
}
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly llm: IApiClient['llm'] = {
    ...
    subscriptionUsage: (payload, signal) => this.callUnary('llm.subscriptionUsage', payload, signal),
  }
